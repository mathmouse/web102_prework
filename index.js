...
contributionsCard.innerHTML = `<p>${totContributions.toLocaleString('en-US')}</p>`;
...
raisedCard.innerHTML = `<p>$${totRaised.toLocaleString('en-US')}</p>`;
...
function filterUnfundedOnly() {
    ...
    let unmetGoal = GAMES_JSON.filter( (game) => {
...
let unmetNum = unmetGoal.length;

// create a string that explains the number of unfunded games using the ternary operator
let unfundedStr = `A total of $${totRaised.toLocaleString('en-US')} has been raised for ${GAMES_JSON.length} games.
    Currently, ${unmetNum} ${unmetNum === 1 ? "game remains" : "games remain"} unfunded. We need
    your help to fund these amazing games!`;

// create a new DOM element containing the template string and append it to the description container
const templateStr = document.createElement("p");
templateStr.innerHTML = `${unfundedStr}`;
...
const topGameCard = document.getElementById("first-game");
const topGame = document.createElement("p");
topGame.innerHTML = `${first.name}`;
topGameCard.appendChild(topGame);
...
runnerUpCard.appendChild(runnerUp);